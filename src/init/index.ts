            ...
            JSON.stringify(
                {
                    ...
                    scripts: {
                        build: config.type === "TypeScript" ? `npx tsc && npx babel ./${config.directories.transpiled} --out-dir ./${config.directories.build}` : `npx babel ./${config.directories.source} --out-dir ./${config.directories.build}`,
